...
#### 三、训练结果  
&nbsp;&nbsp;本方法建立了一个可扩展DNN模型，它包含八个主要模型训练分支，然后扩展了五个新的分支用于迁移学习。多输出DNN神经网络，优势在于它能够利用输入数据中的共享特征，同时处理多个漏洞类型。模型可以通过学习不同漏洞类型之间的相似性和差异性来提高整体的检测性能，并能够同时检测多个漏洞类型。在这个阶段，本方法通过监督式学习来了解标记的字节码数据集中的漏洞。  
